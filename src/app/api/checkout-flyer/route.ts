...
import { FLYER_SIZES, FLYER_QUANTITIES, SIDES, flyerPriceFor, getFlyerProduct } from "../../../../lib/flyers";
...
const Body = z.object({
  ...
  size: z.enum(["5.5x8.5", "8.5x11", "11x17"]),
...
export async function POST(req: NextRequest) {
  try {
    ...
    const product = getFlyerProduct(productId);
    if (!product) return NextResponse.json({ error: "Unknown product" }, { status: 400 });

    const unit = flyerPriceFor(productId, size, quantity, side);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        ...
        price_data: {
          ...
          unit_amount: unit,
          product_data: {
            name: `${product.name} — ${size} ${quantity} flyers (${side === "single" ? "Single" : "Double"}-sided)`,
          },
        },
      }],
      metadata: {
      ...
      cancel_url: `${siteUrl}/shop/flyers`,
    ...
    console.error("[/api/checkout-flyer] ERROR:", err?.message || err);
